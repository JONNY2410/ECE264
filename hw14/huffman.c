...
int _cmp_int(int value1, int value2){	
...
Node* frequency_rank(long int* asciicount){
	...
	for(int i=0;i<256;i++){
		if(asciicount[i]!=0){
			head = _pq_enqueue(&head,i,asciicount[i],_cmp_int);
		}	
	}
	return head;	
}
...
void huffman_code(TreeNode* root, char* array, int index, FILE* fpe){
	if((root->left==NULL)&&(root->right==NULL)){
		...
		array[index] = '0';
		huffman_code(root->left,array,index+1,fpe);	
		array[index] = '1';
		huffman_code(root->right,array,index+1,fpe);
	}
}

void post_traverse(TreeNode* root,FILE* fpc){
	if((root->left==NULL)&&(root->right==NULL)){
		fputc('1',fpc);
		fputc(root->ch,fpc);
		root->mark= 1;
		...
		post_traverse(root->left,fpc);
		post_traverse(root->right,fpc);
		if(((root->left->mark)==1)&&(root->right->mark==1)){
			fputc('0',fpc);
			root->mark = 1;
		}
	}
...
void bit_stream(FILE* tmp, char* header){
	int i = 0;
	int num_bit = 0;
	...
	while(header[i]!='\0'){
		if(header[i]=='0'){
			mask2 = mask2<<(7-num_bit);
			bitdata = bitdata & (mask2^mask);
			mask = 0xff;
			mask2 = 0x01;
			num_bit++;
			if(num_bit==8){
			...
			mask = (mask<<num_bit);
			mask = (mask & header[i])>>num_bit;
			bitdata = bitdata | mask;
			fputc(bitdata,tmp);
			
			mask = 0xff;
			mask = mask>>(8-num_bit);
			mask = (mask & header[i])<<(8-num_bit);
			bitdata = 0;
			bitdata = bitdata | mask;
			mask = 0xff;
		}
